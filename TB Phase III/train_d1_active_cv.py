...
def main() -> None:
    ap = argparse.ArgumentParser(description="D1-only CV on active-only Phase III features")
    ap.add_argument(
        "--npz",
        type=Path,
    ...
    ap.add_argument(
        "--out_dir",
        type=Path,
        default=Path(r"D:\TB Phase III\artifacts\d1_active_cv"),
    )
    ...
    for r in range(args.cv_runs):
        ...
        _ = va  # val kept for split policy consistency
    ...
    out_json = args.out_dir / "d1_active_cv_test_metrics.json"
    out_txt = args.out_dir / "d1_active_cv_test_summary.txt"
    out_csv = args.out_dir / "d1_active_cv_per_run.csv"
    ...
    lines: list[str] = []
    lines.append("D1-only CV (variable split per run, fixed active sample list) — TEST summary")
