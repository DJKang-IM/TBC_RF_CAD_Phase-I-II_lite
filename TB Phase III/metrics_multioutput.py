...
def main() -> None:
    ...
    out = {
        ...
        "labels": [{"key": KEYS[i], "name": NAMES[i]} for i in range(len(KEYS))],
        "sets": [],
    }
    ...
    for feat in args.features:
        ...
        X, Y = load_npz(p)
        pred = rf.predict(X)
        probas = rf.predict_proba(X)  # list per output

        per_label = []
        for j in range(Y.shape[1]):
            y = Y[:, j].astype(int)
            p1 = probas[j][:, 1].astype(float)
            yhat = (p1 >= args.threshold).astype(int)
            ...
            per_label.append(
                {
                    ...
                    "confusion_matrix": {"labels": [0, 1], "matrix": cm.astype(int).tolist()},
                    "eer": eer_from_scores(y, p1),
                }
            )

        out["sets"].append({"name": p.name, "n": int(Y.shape[0]), "per_label": per_label})
