...
from tbc_cad_ver_103.bundle import IntegratedCad, IntegratedCadConfig
from tbc_cad_ver_103.defaults import resolve_hybrid_joblib, resolve_xgb_d6_json


def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        prog="tbc-cad-infer-103",
        description="tbc-cad-ver.1.03 — DICOM to Phase I/II/III JSON (D1 through D6, D5 cavitary, D6 XGB).",
    )
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="D6 XGBoost model .json. If omitted, search under --phase3-root.",
    )
    ap.add_argument(
        "--hybrid-joblib",
        type=Path,
        default=None,
        help="v1.03 hybrid joblib. If omitted, search under --phase3-root.",
    )
    ...
    if hybrid is not None and hybrid.is_file():
        ...
        p3 = Path(args.phase3_root)
        ap.error(
            "Need v1.03 Phase III. Pass --hybrid-joblib or --xgb-d6-json, or place e.g.:\n"
            f"  {p3 / 'artifacts' / 'phase3_v1_03_d5_d6_xgb' / 'phase3_rf_v1_03_d5_d6_xgb_hybrid.joblib'}\n"
            f"  {p3 / 'artifacts' / 'version_1_023' / 'd6_ntm_xgb.json'}"
        )
